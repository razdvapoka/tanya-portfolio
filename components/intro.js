...
class Letters extends React.Component {
  render() {
    const { words, letterBox } = this.props;
...
const Loop = ({ loop, index, width, height, shift, padding, letterBox, velocity, inViewport }) => {
  ...
  useEffect(() => {
    if (path) {
      ...
      const newTimeline = textLoopTimeline(path, loopLetters, velocity, 0);
      newTimeline.play();
  ...
  return (
    <React.Fragment>
      ...
      />
      <Letters words={loop} letterBox={letterBox} />
    </React.Fragment>
  );
};
...
class Intro extends React.Component {
  ...
  render() {
    const { width, height, loops, velocity, shift, innerRef, inViewport, padding } = this.props;

    const { letterBox } = this.state;
    return (
      <IntroBox ref={this.introRef}>
        <Box ref={this.setLetterBox} transform={`translateY(-${THEME.textStyle.intro.fontSize})`} />
        <Svg viewBox={`0 0 ${width} ${height}`} ref={innerRef}>
          ...
          <g transform="translate(320 376)">
            <Text variant="introFixed" as="text">
              tanya
            </Text>
          </g>

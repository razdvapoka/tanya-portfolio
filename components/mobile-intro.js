...
import { MOBILE_INTRO_DURATION, MOBILE_INTRO_LINE_COUNT, WORD_SETS } from "../constants";
...
const Marquee = ({ reversed }) => {
  ...
  const content = WORDS.map((word, wordIndex) => (
    <Text as="span" key={wordIndex} variant="header" mgl={ps("& + &", 4)}>
      {word}
    </Text>
  ));

  return (
    <OVHBox opacity={animation ? 1 : 0} transition="opacity 0.3s ease">
...
const MobileIntro = ({ setIntroHidden, setIntroVisible, ...rest }) => {

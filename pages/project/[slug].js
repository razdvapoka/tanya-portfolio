...
import { getSection, getProject } from "../../content-api.js";

const Column = styled(FlexGrid.Item)(
  ({ bgImage }) =>
    bgImage && {
...
class Project extends React.Component {
  ...
  render() {
    ...
    return (
      <Box pdx={{ all: 4, M: 2 }} pdt={{ all: 5, M: 2 }} tm={palette} minHeight="100vh">
        <ProjectHeader
          project={project}
          palette={palette}
              ...
              >
                <FlexGrid space={{ all: 4, M: 0 }} flexWrap={{ M: "wrap" }}>
                  {row.fields.columns.map((column, columnIndex) => {
                    const { pdx: colPdx = 0, pdy: colPdy = 0, ...colRest } =
                      column.fields.props || {};
                    return (
                      <Column
                        key={columnIndex}
